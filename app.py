...
HK_BLACK_LIST = {
    ...
    "sohu.com", "news.bioon.com", "instagram.com", "163.com"
}

# ==================== V17.1：優化後的 AI 批次審核邏輯 ====================
def ai_batch_region_filter(articles, target_region, white_list):
    if not articles: return []
    confirmed = [a for a in articles if a.get('is_white')]
    pending = [a for a in articles if not a.get('is_white')]
    if not pending: return confirmed
    ...
    for i in range(0, len(pending), batch_size):
        ...
        batch_text = "\n".join([f"ID:{idx} | 媒體:{a['source']} | 標題:{a['title']} | 網址:{a['link']}" for idx, a in enumerate(batch)])
        
        prompt = f"""
你是一個新聞地區審核員。目標是為「{target_region}」盡量保留相關新聞。

判斷標準：
- 如果媒體明顯屬於該地區（白名單已自動通過）
- 如果標題、來源、網址顯示這是該地區發生的事件、或該地區媒體的報導，即使不是核心本地媒體，也應通過
- 國際媒體報導該地區新聞 → 應通過
- 明顯不相關或屬於其他地區的黑名單媒體 → 排除

請回傳所有你認為「值得保留」的 ID 數字，用逗號隔開。
如果全部都值得保留，請回傳 "ALL"。
如果完全沒有，請回傳 "None"。
...
只回傳 ID 或 ALL 或 None，不要解釋。
"""
        try:
            resp = model.generate_content(prompt, safety_settings=safe)
            res_txt = resp.text.strip()
            if "ALL" in res_txt.upper():
                approved_pending.extend(batch)
            elif "None" not in res_txt:
                ids = [int(s) for s in re.findall(r'\d+', res_txt)]
                for v_id in ids:
                    if v_id < len(batch): approved_pending.append(batch[v_id])
        except: 
            approved_pending.extend(batch) # 失敗則放行以防遺漏
    return confirmed + approved_pending
...
def fetch_serper_combined(query, start_date, end_date, gl, hl, white_list, region):
        ...
        try:
            ...
            for i in items:
                ...
                all_results.append({
                    ...
                    "raw_dt": dt, "is_white": check_white(link, '', white_list), "fetch_type": "serper_news"
                })
            time.sleep(0.5)
        except: break
    ...
    try:
        ...
        for i in res.get('organic', []):
            ...
            all_results.append({
                "title": i.get('title', ''), "link": link, "source": source_name,
                "pub_str": dt.strftime("%Y-%m-%d %H:%M"), "raw_dt": dt,
...
# ==================== 3. 主介面 UI ====================
st.set_page_config(page_title="全球 CitizensNews V17.1", layout="wide")
...
if "新聞搜尋" in app_mode:
    ...
    custom_ai_request = ""
    if enable_news_ai:
        custom_ai_request = st.text_area("🎯 AI 自定分析要求 (選填)", placeholder="例如：請重點分析親政府媒體與獨立媒體的報導差異", help="填寫後，AI 將在原有分析基礎上額外執行此要求。")
        
    news_params = (query, region, start_date, end_date)

    if st.button("搜尋", type="primary"):
        with st.status("🔄 正在搜尋並進行 AI 批次地區篩選 (V17.1) ...", expanded=True) as status:
            ...
            raw_list = list(unique.values())
            raw_count = len(raw_list)
            filtered_list = ai_batch_region_filter(raw_list, region, white_list)
            
            st.session_state.news_results = sorted(filtered_list, key=lambda x: x["raw_dt"], reverse=True)
            st.session_state.news_page = 0
            st.session_state.last_news_params = news_params
            status.update(label=f"✅ 挖掘完成！原始 {raw_count} 則 → AI 篩選後 {len(st.session_state.news_results)} 則", state="complete")
            st.rerun()
    ...
    if st.session_state.news_results is not None and st.session_state.last_news_params == news_params:
        ...
        if not res: st.warning("⚠️ 此關鍵字沒有搜到相關新聞")
            ...
            st.success(f"📊 **資料源診斷 (V17.1 AI 已過濾)**：✅白名單: **{white_count}** ｜ 🔴RSS新聞: **{rss_count}** ｜ 🔵Serper新聞: **{serper_count}** ｜ 🌐補充包: **{supp_count}** ｜ 總數: **{len(res)}**")
            ...
            if enable_news_ai:
                ...
                with st.spinner(f"🤖 AI正在閱讀資料... (使用模型: {available_model_path})"):
                    try:
                        ...
                        original_prompt = "請分析以下新聞報導的主要趨勢、各方觀點對比及核心事件整理："
                        if custom_ai_request.strip():
                            final_prompt = f"{original_prompt}\n\n特別注意，使用者有以下額外要求：[{custom_ai_request.strip()}]\n\n資料來源如下：\n{context}"
                        else:
                            final_prompt = f"{original_prompt}\n\n資料來源如下：\n{context}"
                            
                        resp = model.generate_content(final_prompt, safety_settings=safe)
